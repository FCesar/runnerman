...
const runnerman = async (suite, collectionDefinition, environment, iterations) => {
    let guid = null;
    ...
    const promises = new Promise((resolve, reject) => {
        ...
        const filteredCollectionDefinition = filterCollection(collectionDefinition, suite);
        ...
        runner.on('request', (err, summary) => {
            ...
            if (response !== undefined) {
                ...
                const event = new Event({
                    listen: 'test',
                    script: new Script({
                        exec: format('tests["%s"] = %s', guid, result.toString())
                    })
                });
        ...
        runner.on('done', () => {
            console.log(format('Suite %s run complete!', suiteName));
