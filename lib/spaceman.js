...
module.exports = async function (suite, collectionDefinition, environment, iterations) {
    ...
    const promises = new Promise((resolve, reject) => {
        ...
        runner.on('beforeItem', function(err, summary) {
            ...
            {
                var data = suite[summary.item.name].data;

                if (typeof data === "function")
                {
                    data = data(err, summary, this.summary);
                }else{
                    Object.entries(data).forEach(([key, value]) => {
                        if (typeof value === "function")
                        {
                            const newObj = new Object();
                            newObj[key] = value(err, summary, this.summary);
                ...
                Object.assign(this.summary,
                    createGlobalVariable(data, summary.cursor.position, this.summary));
            }
        });
        ...
        runner.on('request', function (err, summary) {
            ...
            if (response !== undefined)
            {
                ...
                {
                    Object.assign(this.summary,
                        createGlobalVariable(JSON.parse(buffer.toString()), summary.cursor.position, this.summary));
                }
            }
        });
