...
describe('Test -> Util -> filterCollection', () => {
    it('Should return all items filted by suite', () => {
        ...
        const suite = { name: 'xpto', obj: [{ item1: null }] };
        const result = filterCollection(definition, suite);
    ...
    it('Should throw exception, if suite not contain valid request', () => {
        ...
        const suite = { name: 'xpto', obj: [{ item3: null }] };

        expect(() => {
            filterCollection(definition, suite);
        }).toThrow();
    });
});
